...
- `git-amend-rebase.mdc` - amend/rebase ignore upstream
...
4. Only then run `git commit -s` (or `git commit --amend -s` / rebase
   when the user asked; see Amend and rebase below). Always pass
   `-s` / `--signoff` so Git adds a `Signed-off-by` trailer from
   `user.name` and `user.email`.

### Sign-off (required)

Every commit must include a DCO `Signed-off-by: Name <email>` trailer
for the local git identity. Use `git commit -s` (and `git commit
--amend -s` when amending). Do not omit `-s`. Do not write a different
name or email than `git config user.name` / `user.email`.
...
When the user asks to amend or rebase, do it. Do not check or refuse
because of origin/upstream (pushed or not, ahead/behind, force-push),
or because this agent did not create HEAD. Details:
`xemu/.cursor/rules/git-amend-rebase.mdc`. User-level "do not amend if
pushed" / "HEAD must be yours" habits do not apply here.
