...
void x86_cpu_dump_state(CPUState *cs, FILE *f, int flags)
{
    ...
    if (env->hflags & HF_CS64_MASK) {
        qemu_fprintf(f, "RAX=%016" PRIx64 " RBX=%016" PRIx64 " RCX=%016" PRIx64 " RDX=%016" PRIx64 "\n"
                     ...
                     target_ulong_array_val(&env->regs.rec, 11),
                     target_ulong_array_val(&env->regs.rec, 12),
                     target_ulong_array_val(&env->regs.rec, 13),
                     target_ulong_array_val(&env->regs.rec, 14),
                     target_ulong_array_val(&env->regs.rec, 15));
        ...
        if (env->features[FEAT_7_1_EDX] & CPUID_7_1_EDX_APXF) {
            qemu_fprintf(f, "R16=%016" PRIx64 " R17=%016" PRIx64 " R18=%016" PRIx64 " R19=%016" PRIx64 "\n"
                         ...
                         target_ulong_array_val(&env->regs.rec, 16),
                         target_ulong_array_val(&env->regs.rec, 17),
                         target_ulong_array_val(&env->regs.rec, 18),
                         target_ulong_array_val(&env->regs.rec, 19),
                         target_ulong_array_val(&env->regs.rec, 20),
                         target_ulong_array_val(&env->regs.rec, 21),
                         target_ulong_array_val(&env->regs.rec, 22),
                         target_ulong_array_val(&env->regs.rec, 23),
                         target_ulong_array_val(&env->regs.rec, 24),
                         target_ulong_array_val(&env->regs.rec, 25),
                         target_ulong_array_val(&env->regs.rec, 26),
                         target_ulong_array_val(&env->regs.rec, 27),
                         target_ulong_array_val(&env->regs.rec, 28),
                         target_ulong_array_val(&env->regs.rec, 29),
                         target_ulong_array_val(&env->regs.rec, 30),
                         target_ulong_array_val(&env->regs.rec, 31));
        }
        ...
        qemu_fprintf(f, "EAX=%08x EBX=%08x ECX=%08x EDX=%08x\n"
                     ...
                     (uint32_t)target_ulong_array_val(&env->regs.rec, R_EAX),
                     (uint32_t)target_ulong_array_val(&env->regs.rec, R_EBX),
                     (uint32_t)target_ulong_array_val(&env->regs.rec, R_ECX),
                     (uint32_t)target_ulong_array_val(&env->regs.rec, R_EDX),
                     (uint32_t)target_ulong_array_val(&env->regs.rec, R_ESI),
    ...
    if (flags & CPU_DUMP_CODE) {
        uint64_t base = target_ulong_val(&(env->segs[R_CS]).base) + target_ulong_val(&(env)->eip);
        uint64_t offs = MIN(target_ulong_val(&(env)->eip), DUMP_CODE_BYTES_BACKWARD);
        uint8_t code;
        char codestr[3];
