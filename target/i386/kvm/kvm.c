...
static void kvm_getput_reg(__u64 *kvm_reg, void *qemu_reg, int set)
{
    if (set) {
        if (target_long_bits() == 64) {
            *kvm_reg = *(uint64_t *)qemu_reg;
        } else {
            *kvm_reg = *(uint32_t *)qemu_reg;
        }
    } else if (target_long_bits() == 64) {
        *(uint64_t *)qemu_reg = *kvm_reg;
    } else {
        *(uint32_t *)qemu_reg = *kvm_reg;
    }
}
