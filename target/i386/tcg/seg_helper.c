...
static void tss_set_busy(CPUX86State *env, int tss_selector, bool value,
                         uintptr_t retaddr)
{
    uint64_t ptr = env->gdt.base + (tss_selector & ~7);
    ...
    if (qemu_loglevel_mask(CPU_LOG_INT)) {
        if ((env->cr[0] & CR0_PE_MASK)) {
            ...
            qemu_log("%6d: v=%02x e=%04x i=%d cpl=%d IP=%04x:%" PRIx64
                     ...
                     env->segs[R_CS].selector, (uint64_t)env->eip,
                     (uint64_t)(env->segs[R_CS].base + env->eip),
                     env->segs[R_SS].selector,
                     (uint64_t)env->regs[R_ESP]);
            if (intno == 0x0e) {
                qemu_log(" CR2=%" PRIx64, (uint64_t)env->cr[2]);
            } else {
                qemu_log(" env->regs[R_EAX]=%" PRIx64,
