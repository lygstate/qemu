...
/* excp_helper.c */
G_NORETURN void x86_raise_exception(CPUX86State *env, int exception_index);
G_NORETURN void x86_raise_exception_ra(CPUX86State *env, int exception_index,
                                       uintptr_t retaddr);
G_NORETURN void x86_raise_exception_err(CPUX86State *env, int exception_index,
                                        int error_code);
G_NORETURN void x86_raise_exception_err_ra(CPUX86State *env, int exception_index,
                                           int error_code, uintptr_t retaddr);

G_NORETURN static inline void raise_exception(CPUX86State *env,
                                              int exception_index)
{
    x86_raise_exception(env, exception_index);
}

G_NORETURN static inline void raise_exception_ra(CPUX86State *env,
                                                 int exception_index,
                                                 uintptr_t retaddr)
{
    x86_raise_exception_ra(env, exception_index, retaddr);
}

G_NORETURN static inline void raise_exception_err(CPUX86State *env,
                                                  int exception_index,
                                                  int error_code)
{
    x86_raise_exception_err(env, exception_index, error_code);
}

G_NORETURN static inline void raise_exception_err_ra(CPUX86State *env,
                                                     int exception_index,
                                                     int error_code,
                                                     uintptr_t retaddr)
{
    x86_raise_exception_err_ra(env, exception_index, error_code, retaddr);
}
