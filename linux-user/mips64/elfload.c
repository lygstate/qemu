...
void elf_core_copy_regs(target_elf_gregset_t *r, const CPUMIPSState *env)
{
    ...
    for (int i = 1; i < 32; i++) {
        regs[i] = tswap64(target_ulong_array_val(&env->active_tc.gpr.rec, i));
    }
    regs[26] = 0;   /* k0 */
    regs[27] = 0;   /* k1 */
    regs[32] = tswap64(target_ulong_array_val(&env->active_tc.LO.rec, 0));
    regs[33] = tswap64(target_ulong_array_val(&env->active_tc.HI.rec, 0));
    regs[34] = tswap64(target_ulong_val(&env->active_tc.PC));
    regs[35] = tswap64(target_ulong_val(&env->CP0_BadVAddr));
